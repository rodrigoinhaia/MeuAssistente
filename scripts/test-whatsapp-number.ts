/**
 * Script de teste para envio de WhatsApp para número específico
 * Testa o envio direto para o número fornecido pelo usuário
 */

import { sendWhatsAppMessage } from '../src/lib/whatsapp/send-message'

async function testWhatsAppNumber() {
  console.log('🧪 Teste de Envio WhatsApp - Número Específico\n')
  ...
  try {
    ...
    if (result) {
      ...
      console.log('\n⚠️  IMPORTANTE:')
      console.log('   - Se não recebeu, verifique se o número está registrado na instância')
      console.log('   - Verifique se a instância está conectada (status "open")')
      console.log('   - O número deve estar no formato correto (com código do país)')
    } else {
